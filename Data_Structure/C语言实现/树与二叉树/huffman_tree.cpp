...
/**
 * 在数组HT[1...n]中选择parent为0且weight最小的两个结点，其序号分别为s1,s2,
 * 并且s1的权小于s2的权
 */
...
Status HuffmanCoding(HuffmanTree &HT, HuffmanCode &HC, int *w, int n)
{
    ...
    /* 从叶子到根逆向求每个字符的赫夫曼编码 */
    HC = (HuffmanCode)malloc(
        (n + 1) * sizeof(char *));  // 分配n个字符编码的头指针向量，0单元未用
    //分配求编码的工作空间，n个字符的编码位数最多为n-1位，最后一位存储'\0'
    ...
    for (i = 1; i <= n; i++) {  // 遂个字符求赫夫曼编码
        ...
        for (current = i, further = HT[i].parent; further != 0;
             current = further,
            further = HT[further].parent) {  // 从叶子开始到根逆向求编码
...
/**
 * 算法6.13，遍历赫夫曼树求编码,HT为已存在的赫夫曼树，HC保存字符编码,
 * n为字符个数,即树的叶子个数
 */
void get_huffmanCode(HuffmanTree HT, HuffmanCode &HC, int n)
{
    ...
    char p;
    HC = (HuffmanCode)malloc(
        (n + 1) * sizeof(char *));  // 分配指向编码的头指针空间，0号单元未用
    code = (char *)malloc(n * sizeof(char));
    codeLen = 0;
    p = 2 * n - 1;  // p指向根结点
